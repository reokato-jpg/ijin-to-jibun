...
def clean_text(s):
    ...
    # 余分な句読点
    s = s.replace("。。", "。")
    # 冒頭の `は、` までを一度カット（フルネーム + カッコ情報 + 「は、」まで）
    #  例: 『フレデリック・ショパン（...、1810年3月1日-1849年10月17日）は、ポーランド出身の...』
    #  → 『ポーランド出身の...』
    m = re.match(r"^[^。]{0,150}?は、", s)
    if m:
        s = s[m.end():]
    # 残った日付カッコ・英語カッコを除去
    s = re.sub(r"（[^）]{0,80}\d{3,4}[^）]{0,80}）", "", s)
    s = re.sub(r"（[A-Za-z][^）]{0,80}）", "", s)
    s = re.sub(r"（[^）]{0,5}\d+月\d+日[^）]{0,30}）", "", s)
    # 先頭の句読点を削る
    s = s.lstrip("、。 ")
    return s.strip()
